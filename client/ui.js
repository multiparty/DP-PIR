...
(function (exports) {
  ...
  L.geoJson(points, {
    ...
    function (feature, latlng) {
      const circle = L.circleMarker(latlng, {radius:7, weight:0.1, fillColor:'red', color:'red', opacity:1, fillOpacity:1});
  ...
  const drawLineBetweenPoints = function (start, stop) {
    ...
    const polyline = L.polyline(latlngs, {color: 'midnightblue'});
    path.push(polyline);
  ...
  exports.drawPath = function (points) {
    pointCount = 0;
    for (let jump = 0; jump < points.length; jump++) {
      if (jump < points.length-1) {
        const source = exports.getPointById(points[jump])['coordinates'].slice().reverse();
        const dest = exports.getPointById(points[jump+1])['coordinates'].slice().reverse();
