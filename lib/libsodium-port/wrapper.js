(function (exports, node) {
  ...
  var maxUniform; // for rejection sampling;
  // random numbers in [1, exports.prime)
  exports.randomScalar = function () {
    if (maxUniform == null) {
      var max = new _BN(2);
      var pow = new _BN(sodiumSumo.libsodium._crypto_scalarmult_curve25519_scalarbytes()).mul(new _BN(8));
      max = max.pow(pow);
      maxUniform = max.div(exports.prime); // integer division, floored
      maxUniform = maxUniform.mul(exports.prime);
    }
    ...
    do {
      var randomBuf;
      if (node) {
        randomBuf = lib.randomScalar().slice(0);
        randomBuf = new Uint8Array(randomBuf);
      } else {
        randomBuf = sodiumSumo.randombytes_buf(sodiumSumo.libsodium._crypto_scalarmult_ed25519_scalarbytes());
        console.log(randomBuf.length);
      }

      var randomBN = exports.bytesToBN(randomBuf);
      reducedRandom = randomBN.mod(maxUniform);
    } while (!randomBN.lt(maxUniform) || reducedRandom.isZero());
    return reducedRandom;
  };
