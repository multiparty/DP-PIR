...
bool Shuffler::Initialize(uint32_t machine_id, uint32_t given_size) {
  ...
  // Resize vectors.
  this->shuffled_queries_.resize(this->batch_size_);
...
// noise queries generated by this machine, but sent to that machine.
std::vector<uint32_t> Shuffler::OutgoingNoiseQueriesCount(uint32_t noise_size) {
  std::vector<uint32_t> result(this->parallelism_ + 1, 0);
  for (uint32_t i = 0; i < noise_size; i++) {
    uint32_t target_machine_id = this->query_machine_ids_.at(i);
...
bool Shuffler::ShuffleQuery(uint32_t machine_id, const types::Query &query) {
  // Find index.
  auto &[i, nested_map] = this->query_indices_.at(machine_id);
  uint32_t index = nested_map.at(i++);
  // Copy into index.
  this->shuffled_queries_.at(index) = query;
...
types::Query &Shuffler::NextQuery() {
