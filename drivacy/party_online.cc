...
absl::Status Setup(uint32_t party_id, uint32_t machine_id, uint32_t table_size,
  ...
  if (party_id == 1) {
    drivacy::parties::online::HeadParty party(party_id, machine_id, config,
                                              table, span, cutoff, batch_size);
    party.Start();
  } else if (party_id == config.parties()) {
    drivacy::parties::online::BackendParty party(party_id, machine_id, config,
                                                 table, span, cutoff);
    party.Start();
  } else {
    drivacy::parties::online::Party party(party_id, machine_id, config, table,
                                          span, cutoff);
    party.Start();
  }
